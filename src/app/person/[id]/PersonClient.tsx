...
const PEOPLE: Record<string, {
  ...
  liam: {
    name: "Liam Johnson", birthYear: 1986,
    bio: "Software engineer based in New York City. Lover of jazz, coffee, and old maps. Studying our family's migration from Ireland to the US in the 1880s.",
...
const DEFAULT_PERSON = {
  name: "Unknown Person", birthYear: 1970,
  bio: "No bio added yet. Invite this person to claim their node.",
  claimed: false, verifiedCount: 0, generation: "Unknown", gender: "other",
  media: [] as Array<{ id: string; type: string; caption: string; visibility: string; timeLock?: string }>,
  relatives: [] as Array<{ id: string; name: string; relation: string }>,
};

function MediaCard({ item }: { item: { id: string; type: string; caption: string; visibility: string; timeLock?: string } }) {
  const colors: Record<string, string> = { PUBLIC: "#10b981", FAMILY: "#d4a843", LOCKED: "#8b5cf6" };
      ...
      <div className="relative overflow-hidden"
        ...
        {item.visibility === "LOCKED" ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <p className="text-xs text-violet-400 font-semibold">Time-Locked</p>
            {item.timeLock && <p className="text-xs text-gray-500 mt-1">Unlocks {item.timeLock}</p>}
          </div>
        ) : item.type === "VIDEO" ? (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-5xl opacity-30">🎬</div>
            <div className="absolute w-14 h-14 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform"
              style={{ background: "rgba(212,168,67,0.2)", border: "2px solid rgba(212,168,67,0.4)" }}>▶</div>
          </div>
        ) : item.type === "TEXT" ? (
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <div className="text-5xl opacity-20 absolute">📝</div>
            <p className="text-xs text-gray-400 text-center line-clamp-4 relative z-10 italic leading-relaxed">{item.caption}</p>
          </div>
        ) : (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-5xl opacity-30">🖼</div>
            <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, rgba(212,168,67,0.1), rgba(139,92,246,0.1))" }} />
          </div>
        )}
        <div className="absolute top-2 left-2">
          <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: "rgba(12,10,26,0.8)", color: "#9ca3af" }}>{item.type}</span>
        </div>
        <div className="absolute top-2 right-2">
          <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: `${color}20`, color }}>{item.visibility}</span>
        </div>
      </div>
      <div className="p-3">
        <p className="text-xs text-gray-300 font-medium line-clamp-2">{item.caption}</p>
        {item.timeLock && <p className="text-xs text-violet-400 mt-1">⏳ Unlocks {item.timeLock}</p>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen">
      <header className="glass border-b border-amber-400/10 px-6 py-4 flex items-center gap-3">
        ...
        <Link href="/family/johnson/tree" className="text-gray-400 hover:text-white text-sm">Johnson Family Tree</Link>
        <span className="text-gray-600">/</span>
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            <div className="relative flex-shrink-0">
              <div className="w-28 h-28 rounded-2xl flex items-center justify-center text-4xl font-black animate-pulse-glow"
                style={{ background: "linear-gradient(135deg, rgba(212,168,67,0.2), rgba(139,92,246,0.2))", border: "2px solid rgba(212,168,67,0.3)", color: "#d4a843" }}>
                {initials}
              </div>
              {person.claimed && (
                <div className="absolute -bottom-2 -right-2 w-8 h-8 rounded-full bg-green-500 flex items-center justify-center text-xs border-2 border-[#0c0a1a]">✓</div>
              )}
            </div>
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-3 text-sm text-gray-400 mb-4">
                ...
                {person.verifiedCount > 0 && (
                  <><span className="text-gray-600">·</span>
                  <span className="text-amber-400">✦ Verified by {person.verifiedCount} relative{person.verifiedCount !== 1 ? "s" : ""}</span></>
                )}
              </div>
              <p className="text-gray-300 leading-relaxed mb-5 max-w-2xl">{person.bio}</p>
              <div className="flex flex-wrap gap-3">
                {!person.claimed && (
                  <button onClick={() => setShowClaim(true)} className="btn-gold px-5 py-2.5 rounded-xl text-sm font-bold">
                    Claim This Profile →
                  </button>
                )}
                <button onClick={() => setShowVerify(true)} className="btn-outline-gold px-5 py-2.5 rounded-xl text-sm font-semibold">
        ...
        <div className="flex gap-1 mb-6 glass rounded-xl p-1 w-fit">
          {(["media", "relatives", "timeline"] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              style={{ background: activeTab === tab ? "rgba(212,168,67,0.15)" : "transparent", color: activeTab === tab ? "#d4a843" : "#9ca3af" }}>
              {tab === "media" ? `📸 Media (${person.media.length})` : tab === "relatives" ? `👥 Relatives (${person.relatives.length})` : "📅 Timeline"}
            </button>
          ))}
        </div>
          ...
          <div className="animate-fadeIn">
            {person.media.length > 0 ? (
              ...
              <div className="text-center py-20">
                ...
                <Link href={`/person/${id}/upload`} className="btn-gold px-6 py-3 rounded-xl text-sm font-bold inline-block">Upload First Media →</Link>
              </div>
            )}
            <div className="mt-6 glass rounded-xl p-4 flex flex-wrap gap-4 text-xs text-gray-500">
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-green-400 inline-block" />PUBLIC</span>
              <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-amber-400 inline-block" />FAMILY only</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="glass rounded-3xl p-8 w-full max-w-sm animate-fadeInUp">
            ...
            <p className="text-gray-400 text-sm mb-6">
              Confirm you personally know this person. Currently verified by{" "}
              <strong className="text-amber-400">{person.verifiedCount}</strong> relative{person.verifiedCount !== 1 ? "s" : ""}.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setShowVerify(false)} className="flex-1 btn-outline-gold py-3 rounded-xl text-sm font-semibold">Cancel</button>
              <button onClick={() => setShowVerify(false)} className="flex-1 btn-gold py-3 rounded-xl text-sm font-bold">✅ Verify</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showClaim && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }}>
          <div className="glass rounded-3xl p-8 w-full max-w-md animate-fadeInUp">
            ...
            <p className="text-gray-400 text-sm mb-4">Claiming links this node to your account. Relatives will verify you.</p>
            <div className="space-y-4 mb-6">
              ...
              <p className="text-xs text-gray-600">Once 2+ relatives confirm, you receive the verified badge.</p>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowClaim(false)} className="flex-1 btn-outline-gold py-3 rounded-xl text-sm font-semibold">Cancel</button>
              <button onClick={() => setShowClaim(false)} className="flex-1 btn-gold py-3 rounded-xl text-sm font-bold">Send Claim Request →</button>
            </div>
          </div>
        </div>
      )}
