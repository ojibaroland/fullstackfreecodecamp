...
export default function UploadClient({ id }: { id: string }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="glass rounded-3xl p-12 text-center max-w-md w-full animate-fadeInUp">
          ...
          <h2 className="text-2xl font-black mb-3">{visibility === "LOCKED" ? "Memory Locked" : "Upload Successful"}</h2>
          ...
          {visibility === "LOCKED" && (
            <div className="glass-light rounded-xl p-4 mb-6 mt-4">
              <p className="text-xs text-violet-400 font-mono leading-relaxed">
                ...
                Your content will NOT be accessible before {releaseDate}.
              </p>
            </div>
          )}
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-2xl mx-auto px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-sm">Add a photo, video, or written memory. Locked items are encrypted immediately.</p>
        </div>
        ...
        <form onSubmit={handleUpload} className="space-y-6">
          ...
          <div className="glass rounded-2xl p-6">
            ...
            {mediaType === "TEXT" ? (
              <textarea value={textContent} onChange={(e) => setTextContent(e.target.value)} rows={8}
                placeholder={`Write your memory, letter, or story here…\n\nThis could be a letter to a grandchild, a recipe, or anything you want preserved.`}
              ...
              <div onClick={() => fileRef.current?.click()} onDrop={handleDrop}
                ...
                style={{ border: `2px dashed ${dragOver ? "rgba(212,168,67,0.6)" : "rgba(212,168,67,0.2)"}`, background: dragOver ? "rgba(212,168,67,0.05)" : "rgba(255,255,255,0.02)" }}>
                {file ? (
                  <div>
                    ...
                    <button type="button" onClick={(e) => { e.stopPropagation(); setFile(null); }}
                      className="text-xs text-gray-500 hover:text-rose-400 mt-2 transition-colors">Remove</button>
                  </div>
                ) : (
                  <div>
                    <div className="text-4xl mb-3">{mediaType === "IMAGE" ? "🖼" : "🎬"}</div>
                    <p className="text-gray-400 text-sm mb-1">Drop here or <span className="text-amber-400">browse</span></p>
                    <p className="text-gray-600 text-xs">{mediaType === "IMAGE" ? "PNG, JPG, WEBP up to 50MB" : "MP4, MOV, WebM up to 2GB"}</p>
                  </div>
                )}
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <input value={caption} onChange={(e) => setCaption(e.target.value)}
              placeholder={visibility === "LOCKED" ? '"Letter to Ella — Open on 18th Birthday"' : '"Summer vacation, Cape Cod 2019"'}
              ...
              {(["PUBLIC", "FAMILY", "LOCKED"] as const).map((v) => {
                ...
                return (
                  <label key={v} className="flex items-start gap-4 p-4 rounded-xl cursor-pointer transition-all"
                    style={{ background: visibility === v ? `${cfg.color}10` : "rgba(255,255,255,0.02)", border: `1px solid ${visibility === v ? `${cfg.color}40` : "rgba(255,255,255,0.06)"}` }}>
                    <input type="radio" name="visibility" value={v} checked={visibility === v} onChange={() => setVisibility(v)} className="mt-0.5 accent-amber-400" />
                    <div>
          ...
          {visibility === "LOCKED" && (
            <div className="glass rounded-2xl p-6 border animate-fadeInUp" style={{ borderColor: "rgba(139,92,246,0.3)" }}>
              <label className="block text-xs font-semibold text-gray-400 mb-3 uppercase tracking-wider">Release Date & Time ⏳</label>
              ...
              <div className="mt-4 glass-light rounded-xl p-4">
                <p className="text-xs text-violet-300 leading-relaxed">
                  🔐 <strong>AES-GCM encryption</strong> will be applied immediately upon upload. Key held server-side.
                </p>
                <p className="text-xs text-gray-600 mt-2">
                  ⚠ <em>Company-held key model</em> — trustless drand tlock coming soon.
                </p>
              </div>
            </div>
          )}

          <button type="submit" disabled={uploading || (mediaType !== "TEXT" && !file) || !caption}
            ...
            {uploading ? (
              ...
              </svg>{visibility === "LOCKED" ? "Encrypting & Uploading…" : "Uploading to archive…"}</>
